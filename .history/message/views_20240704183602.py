...
def message_home(request,rest_id,user_id):
    ...
    sender=request.user 
    message_obj=message.objects.filter(Q(restaurant=restaurant) & Q(sender=sender) & receiver Q(receiver=receiver))
    ...
    if request.method == "POST":
        ...
        message_obj.save()
        return redirect("message-home",rest_id=restaurant.id ,user_id=sender.id)
