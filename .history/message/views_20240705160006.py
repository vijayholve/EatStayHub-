...
def message_home(request,rest_id):
    ...
    message_obj=message.objects.filter( Q(sender=receiver) &  Q(receiver=receiver))
...
def restaurant_owner_reply(request,receiver_id , sender_id):
    sender=User.objects.get(id=sender_id) 
    receiver=User.objects.get(id=receiver_id) 
    message_obj=message.objects.filter(Q(sender=receiver) &  Q(receiver=sender))
    if request.method == "POST":
        message_text=request.POST.get("message")
        message_obj=message.objects.create(
            receiver=receiver,
        ...
        message_obj.save()
        return redirect("message-owner",receiver_id=receiver.id ,sender_id=sender.id)
    context={"message_obj":message_obj,"sender":sender,"receiver":receiver}
