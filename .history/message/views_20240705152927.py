...
def message_home(request,rest_id):
    ...
    receiver=User.objects.get(id=restaurant.user)  
...
def restaurant_owner_reply(request,receiver_id , sender_id):
    ...
    message_obj=message.objects.filter(Q(restaurant=restaurant) & Q(sender=receiver) &  Q(receiver=sender))
    ...
    if request.method == "POST":
        ...
        return redirect("message-home",rest_id=restaurant.id)
    context={"message_obj":message_obj,"sender":sender,"receiver":receiver}
    return render(request,"message/message_home_.html",context)
