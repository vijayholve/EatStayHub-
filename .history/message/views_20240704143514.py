...
def message_home(request,pk):
    ...
    sender=request.user
    message_obj=message.objects.filter(Q(sender=sender) & Q(aaaaaaaaaasreceiver=receiver))[:1]
    user_obj=User.objects.filter(Q(message__sender=sender) & Q(message__receiver=receiver))
