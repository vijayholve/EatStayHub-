...
def all_reciever_message(request,id):
    ...
    sender_list=message.objects.filter(Q(restaurant=restaurant) & Q(def FUNCTIONNAME_pre_init_receiver(sender, instance, *args, **kwargs):
        pass
    
    
    pre_init.connect(FUNCTIONNAME_pre_init_receiver, sender=MODELNAME))).distinct()
